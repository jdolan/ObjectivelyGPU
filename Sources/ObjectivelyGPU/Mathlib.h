...
/**
 * @brief Two-component single-precision vector. Component layout matches GLSL `vec2`.
 */
typedef union {
...
/**
 * @return The ones vector `(1, 1)`.
 */
static inline vec2 vec2_one(void) {
	return vec2_new(1.f, 1.f);
}
...
/**
 * @return The component-wise product `a * b`.
 */
...
/**
 * @return The unit vector of `v`, or `v` if zero-length.
 */
...
static inline vec2 vec2_min(vec2 a, vec2 b) {
	return vec2_new(a.x < b.x ? a.x : b.x, a.y < b.y ? a.y : b.y);
}
...
/**
 * @brief Three-component single-precision vector. Component layout matches GLSL `vec3`.
...
/**
 * @return The ones vector `(1, 1, 1)`.
 */
static inline vec3 vec3_one(void) {
	return vec3_new(1.f, 1.f, 1.f);
}
...
static inline vec3 vec3_min(vec3 a, vec3 b) {
	return vec3_new(a.x < b.x ? a.x : b.x, a.y < b.y ? a.y : b.y, a.z < b.z ? a.z : b.z);
}
...
/**
 * @brief Four-component single-precision vector. Component layout matches GLSL `vec4`.
...
typedef union {
	...
	/**
	 * @brief Swizzle to `vec3`.
	 */
	vec3 xyz;

	/**
	 * @brief Swizzles to `vec2`.
	 */
	struct {
		vec2 xy;
		vec2 zw;
...
/**
 * @return The ones vector `(1, 1, 1, 1)`.
 */
static inline vec4 vec4_one(void) {
	return vec4_new(1.f, 1.f, 1.f, 1.f);
}
 ...
 * @return The squared length of `v`.
 */
static inline float vec4_length_squared(vec4 v) {
	return vec4_dot(v, v);
}
...
static inline vec4 vec4_min(vec4 a, vec4 b) {
	return vec4_new(a.x < b.x ? a.x : b.x, a.y < b.y ? a.y : b.y, a.z < b.z ? a.z : b.z, a.w < b.w ? a.w : b.w);
}
...
/**
 * @brief Column-major 4×4 single-precision matrix. Layout matches GLSL `mat4`.
...
typedef union {
	...
	/**
	 * @brief Column-major 2D array accessor. `m[col][row]`.
	 */
	float m[4][4];
